...
def CheckTransaction(q, p, g):
    ...
    payer_pk = int(transaction[6][25:])  # payer's publick key
    sign_s  = int(transaction[1][15:])   # signature for the transaction (part s)
    sign_h = int(transaction[2][15:])    # signature for the transaction (part h)

    message2sign = '\n'.join(transaction[3:7])+'\n'

    if DS.SignVer(message2sign.encode('utf-8'), sign_s, sign_h, q, p, g, payer_pk)==0:
        return 0
    else:
        return -1
...
def CheckBlockofTransactions():
    ...
    if len(tx_block)%7 != 0:
        ...
        for i in range(0, tx_block_count):
            pk = int(tx_block[i*7+6][25:])
            s = int(tx_block[i*7+1][15:])
            h = int(tx_block[i*7+2][15:])
            tx = "".join(tx_block[i*7+3: i*7+7])
            result[i] = DS.SignVer(tx.encode('UTF-8'), s, h, q, p, g, pk)
    return result        
